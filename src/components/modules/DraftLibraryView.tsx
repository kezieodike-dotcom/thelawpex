    ...
    <div className="bg-neutral-950 text-white min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-neutral-900 border border-yellow-500/30 rounded-2xl p-6 sm:p-8 mb-8 shadow-xl">
          ...
          <h1 className="text-2xl sm:text-4xl font-black font-serif text-white mt-2">Legal Draft & Process Library</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredDrafts.map((draft) => (
            <div
              key={draft.id}
              className="bg-neutral-900 border border-neutral-800 hover:border-yellow-500/40 p-6 rounded-2xl transition shadow-xl space-y-4 flex flex-col justify-between group"
            >
              <div className="space-y-2">
                ...
                <h3 className="text-base font-bold font-serif text-white group-hover:text-yellow-400 transition">
                  {draft.title}
                </h3>

                <p className="text-xs text-neutral-400 line-clamp-2 leading-relaxed">{draft.description}</p>
              </div>

              <div className="pt-3 border-t border-neutral-800 flex items-center justify-between gap-2">
                <button
                  onClick={() => onCustomizeDraft(draft)}
                  className="w-full bg-yellow-400 hover:bg-yellow-300 text-neutral-950 font-bold text-xs py-2.5 rounded-xl transition flex items-center justify-center gap-1.5 shadow-md shadow-yellow-500/10"
                >
